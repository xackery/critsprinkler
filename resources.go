...
func newUIResources() (*uiResources, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("new button resources: %w", err)
	}

	checkbox, err := newCheckboxResources()
	if err != nil {
		return nil, fmt.Errorf("new checkbox resources: %w", err)
	}

	comboButton, err := newComboButtonResources(fonts)
	if err != nil {
		return nil, fmt.Errorf("new combo button resources: %w", err)
	}

	list, err := newListResources(fonts)
	if err != nil {
		return nil, fmt.Errorf("new list resources: %w", err)
	}

	slider, err := newSliderResources()
	if err != nil {
		return nil, fmt.Errorf("new slider resources: %w", err)
	}

	progressBar, err := newProgressBarResources()
	if err != nil {
		return nil, fmt.Errorf("new progress bar resources: %w", err)
	}

	panel, err := newPanelResources()
	if err != nil {
		return nil, fmt.Errorf("new panel resources: %w", err)
	}

	tabBook, err := newTabBookResources(fonts)
	if err != nil {
		return nil, fmt.Errorf("new tab book resources: %w", err)
	}

	header, err := newHeaderResources(fonts)
	if err != nil {
		return nil, fmt.Errorf("new header resources: %w", err)
	}

	textInput, err := newTextInputResources(fonts)
	if err != nil {
		return nil, fmt.Errorf("new text input resources: %w", err)
	}
	textArea, err := newTextAreaResources(fonts)
	if err != nil {
		return nil, fmt.Errorf("new text area resources: %w", err)
	}
	toolTip, err := newToolTipResources(fonts)
	if err != nil {
		return nil, fmt.Errorf("new tool tip resources: %w", err)
	}
...
func newButtonResources(fonts *fonts) (*buttonResources, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("load image nine slice: %w", err)
	}
